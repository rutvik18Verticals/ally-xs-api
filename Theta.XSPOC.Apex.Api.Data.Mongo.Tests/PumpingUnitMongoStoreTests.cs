...
{
    [TestClass]
    public class PumpingUnitMongoStoreTests
    {
        ...
        {
            _ = new PumpingUnitMongoStore(null, null);
        }

        [TestMethod]
        public void GetUnitNamesTest()
        {
            ...
            mockCollection.Setup(m => m.FindSync(It.IsAny<FilterDefinition<Lookup>>(),
                ...
                .Returns(TestUtility.GetMockMongoData<Lookup>("Lookup", (int)LookupTypes.PumpingUnit));
            ...
            var dataStore = new PumpingUnitMongoStore(mockDatabase.Object, mockThetaLoggerFactory.Object);

            var nodeIds = new List<string> { "001 DigiUltra", "Well1" };
            var result = dataStore.GetUnitNames(nodeIds, "correlationId1");
