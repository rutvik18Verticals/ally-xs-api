...
{
    /// <summary>
    /// Transmits transaction data to a RabbitMQ queue to be monitored by the listner so the lister can
    /// send data back to the web socket.
    /// </summary>
    public class LegacyTransactionMonitorPubliser :
        ExchangePublisherBase<ProcessDataUpdateContract, Responsibility, Loggers, LoggingModel>
    {
        ...
        private const string ROUTING_KEY = "XSPOC.Legacy.Transactions";
        ...
        /// </exception>
        public LegacyTransactionMonitorPubliser(IConfiguration config, IConnectionFactory connectionFactory,
        ...
        /// <seealso cref="IPublishMessage{T}"/>. This responsibility is to publish the transaction id to be
        /// monitored by the listner to push data back to the web socket.
        /// </summary>
        public override Responsibility Responsibility => Responsibility.PublishTransationIdToListener;
        ...
        protected override void SetConfigurationOnInit()
        {
            TopicExchangeName = Config.GetSection("TopicExchanges:DataStoreUpdatesExchange").Value ??
                                TOPIC_EXCHANGE_NAME;
            BaseRoutingKey = Config.GetSection("TopicExchanges:LegacyTransactionRoutingKey").Value ?? ROUTING_KEY;
